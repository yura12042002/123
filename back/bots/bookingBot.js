...
bot.on("callback_query", async (query) => {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      await booking.save();
      await bot.sendMessage(chatId, `✅ Бронь подтверждена для *${booking.name}*`, { parse_mode: "Markdown" });
    }

    if (action === "reject") {
      booking.status = "rejected";
      await booking.save();
      await bot.sendMessage(chatId, `❌ Бронь отклонена для *${booking.name}*`, { parse_mode: "Markdown" });
    }
